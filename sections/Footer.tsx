    ...
    return (
        <footer className={" w-full"}>
            <AnimatePresence>
                {
                    showUpButton &&
                    <motion.button aria-label="Back to top" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} onClick={() => window.scrollTo({ top: 0 })} className={cn('fixed bottom-10 right-20 rounded-full', buttonVariants({ variant: "default", size: "icon" }))}>
                        <ArrowUp />
                    </motion.button>
                }
            </AnimatePresence>
