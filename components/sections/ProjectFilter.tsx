...
export default function ProjectFilter({activeFilter} : {activeFilter?: string}) {
    ...
    return (
        <div className={'text-white-100 flex-row-center gap-6'}>
            {
                filters.map(filter => (
                    <Button onClick={() => handleSelectedFilter(filter)} key={filter} variant={'tertiary'} className={selectedFilter.toLowerCase() === filter.toLowerCase() ? 'bg-neutral-dark-60' : ''}>{filter}</Button>
                ))
            }
        </div>
    )
